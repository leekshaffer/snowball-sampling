...
for (k.par in disp_params) {
  ...
  plot_top <- ggplot(data=Infections2AJE, aes(x=median, y=Y)) +
    ...
    scale_x_continuous(name="Median", limit=c(-450,800), ## Limit is big enough to allow the text labels
    ...
    geom_text(aes(x=-450, y=Y, label=label, hjust="left"), inherit.aes=FALSE, size=8/ggplot2:::.pt) + ## Left side text labels
    annotate("text", x=-230, y=5.2, label='underline("Day and Sampling")', parse=TRUE, hjust="center", size=8/ggplot2:::.pt) + 
    geom_text(aes(x=510, y=Y, label=Vals, hjust="left"), inherit.aes=FALSE, size=8/ggplot2:::.pt) + ## Right side text labels
    annotate("text", x=660, y=5.2, label='underline("Median (IQR)")', parse=TRUE, hjust="center", size=8/ggplot2:::.pt) +
    annotate("segment", x=-7, xend=507, y=0, yend=0, color="black", size=2) ## Bottom axis that ends at break labels
  ...
  plot_mid <- ggplot(data=Prop.Est.AJE, aes(x=median, y=Y)) +
    ...
    scale_x_continuous(name="Median", limit=c(-.12,.17), ## Limit is big enough to allow the text labels
                       breaks=seq(0,.08,by=.04), labels=as.character(seq(0,.08,by=.04))) + ## Breaks & labels only where there is actual data
    scale_y_continuous(limit=c(0,5.6), expand=expansion(add=c(0,0))) + ## Needs to match Y vector, but with some space above and below
    geom_text(aes(x=-.12, y=Y, label=label, hjust="left"), inherit.aes=FALSE, size=8/ggplot2:::.pt) + ## Left side text labels
    annotate("text", x=-.067, y=5.2, label='underline("Day and Sampling")', parse=TRUE, hjust="center", size=8/ggplot2:::.pt) + 
    geom_text(aes(x=.082, y=Y, label=Vals, hjust="left"), inherit.aes=FALSE, size=8/ggplot2:::.pt) + ## Right side text labels
    annotate("text", x=.13, y=5.2, label='underline("Median (IQR)")', parse=TRUE, hjust="center", size=8/ggplot2:::.pt) +
    annotate("segment", x=-.0015, xend=.0815, y=0, yend=0, color="black", size=2) + ## Bottom axis that ends at break labels
    geom_segment(x=.05, xend=.05, y=0, yend=4.8)
  ...
  plot_bot <- ggplot(data=Prop.MSE.AJE, aes(x=RMSE, y=Y)) +
    ...
    scale_x_continuous(name="Root Mean Squared Error", limit=c(-.035,.06), ## Limit is big enough to allow the text labels
                       breaks=seq(0,.05,by=.025), labels=as.character(seq(0,.05,by=.025))) + ## Breaks & labels only where there is actual data
    scale_y_continuous(limit=c(0,5.6), expand=expansion(add=c(0,0))) + ## Needs to match Y vector, but with some space above and below
    geom_text(aes(x=-.035, y=Y, label=label, hjust="left"), inherit.aes=FALSE, size=8/ggplot2:::.pt) + ## Left side text labels
    annotate("text", x=-.0175, y=5.2, label='underline("Day and Sampling")', parse=TRUE, hjust="center", size=8/ggplot2:::.pt) + 
    geom_text(aes(x=.051, y=Y, label=Vals, hjust="left"), inherit.aes=FALSE, size=8/ggplot2:::.pt) + ## Right side text labels
    annotate("text", x=.051, y=5.2, label='underline("RMSE")', parse=TRUE, hjust="left", size=8/ggplot2:::.pt) +
  ...
  p.2.top <- ggplot(data=Prop.CIW.AJE, aes(x=median, y=Y)) +
    ...
    scale_x_continuous(name="Median", limit=c(-0.6,1), ## Limit is big enough to allow the text labels
                       breaks=seq(0,0.5,by=.1), labels=as.character(seq(0,0.5,by=.1))) + ## Breaks & labels only where there is actual data
    scale_y_continuous(limit=c(0,5.6), expand=expansion(add=c(0,0))) + ## Needs to match Y vector, but with some space above and below
    geom_text(aes(x=-0.6, y=Y, label=label, hjust="left"), inherit.aes=FALSE, size=8/ggplot2:::.pt) + ## Left side text labels
    annotate("text", x=-.32, y=5.2, label='underline("Day and Sampling")', parse=TRUE, hjust="center", size=8/ggplot2:::.pt) + 
    geom_text(aes(x=.51, y=Y, label=Vals, hjust="left"), inherit.aes=FALSE, size=8/ggplot2:::.pt) + ## Right side text labels
    annotate("text", x=.77, y=5.2, label='underline("Median (IQR)")', parse=TRUE, hjust="center", size=8/ggplot2:::.pt) +
    annotate("segment", x=-.008, xend=.508, y=0, yend=0, color="black", size=2) ## Bottom axis that ends at break labels
  ...
  p.2.bot <- ggplot(data=Prop.Cov.AJE, aes(x=Cov, y=Y)) +
    ...
    annotate("segment", x=89.85, xend=100.15, y=0, yend=0, color="black", size=2) + ## Bottom axis that ends at break labels
    geom_segment(x=95, xend=95, y=0, yend=4.8)
